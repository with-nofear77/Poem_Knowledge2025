...
def call_with_stream(word):
    messages = [
        {'role': 'user', 'content': word}]
    responses = dashscope.Generation.call("qwen-plus",  # 调用的模型接口
                                          messages=messages,
                                          result_format='message',  # set the result to be "message"  format.
                                          stream=False,  # 是否开启流式输出，不开启设置False
                                          incremental_output=False  # 是否开启流式输出，不开启设置False
                                          )
...
def poemer_produce():
    ...
    for it in data:
        ...
        if i < 8 :
            ...
            sql = "select count(*) from author where  author='%s' and src <> 'https://img2.baidu.com/it/u=1080228242,3641064450&fm=253&fmt=auto&app=120&f=JPEG?w=380&h=285'"
            connect = None
            ...
            if exist_count[0][0] > 0:
                sql = "select src from author where  author='%s' and  src<>'https://img2.baidu.com/it/u=1080228242,3641064450&fm=253&fmt=auto&app=120&f=JPEG?w=380&h=285' "
                cur = connect.cursor()
                print(sql % name)
                cur.execute(sql % name)
                src_data = cur.fetchmany(1)
                print(src_data)
                #src = src_data[0][0]
                result['src']= src_data[0][0]
            else:
                #src='https://img0.baidu.com/it/u=4085259550,1835585026&fm=253&fmt=auto&app=138&f=PNG?w=654&h=500'
                sql = "SELECT src FROM author where src<>'https://img2.baidu.com/it/u=1080228242,3641064450&fm=253&fmt=auto&app=120&f=JPEG?w=380&h=285' ORDER BY RAND() LIMIT 1;"
                cur = connect.cursor()
                #print(sql % name)
                cur.execute(sql)
                src_data = cur.fetchmany(1)
                ...
                # src = src_data[0][0]
                result['src'] = src_data[0][0]
                #result['src']='https://img0.baidu.com/it/u=4085259550,1835585026&fm=253&fmt=auto&app=138&f=PNG?w=654&h=500'
                #随机取一张

            #result['src'] = src
            #print('最终图片',src)
            print('最终图片',result['src'])
            #print('最终图片',src)
...
def poemer_things():
    ...
    db = MySqLHelper()
    sql='select * from author where author=%s'
    ret=db.selectone(sql=sql,param=name)
    jsonData = []
    result={}
    #produce=ret[1]
    begin_time=ret[6]
    ...
    bg = int(begin_time.replace('年', ''))
    ed = int(end_time.replace('年', ''))
    #produce=str(data[0].get('p.produce')).split('。')[0]+"。"
    if data[0].get('p.produce') ==None :
        produce='无'
    else:
        produce=str(data[0].get('p.produce')).split('。')[0]+"。"
...
def get_poemtime_about_poem():
    ...
    bg_time=''
    for it in desty:
        sql = "select * from " + it + " where title like '%" + poem_name + "%' or qu_name like '%"+poem_name+"%' or zhu like '%"+poem_name+"%' limit 1"
        ret = db.selectone(sql=sql)
        if ret!=None:
            author=ret[2]
...
def send_question():
    ...
    que=str(que)
    print("我的问题："+que)
    answer = call_with_stream('请详细回答：'+que)
